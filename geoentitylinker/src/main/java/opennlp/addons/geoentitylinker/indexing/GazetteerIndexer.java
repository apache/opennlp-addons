...
public class GazetteerIndexer {
  ...
  public void index(File geonamesData, File geoNamesCountryInfo, File geonamesAdmin1CodesASCII,
      File usgsDataFile,
                    File usgsGovUnitsFile, File outputIndexDir, File outputCountryContextFile,
                        File regionsFile)
          throws IOException {
    ...
    try (IndexWriter w = new IndexWriter(index, config)) {
      ...
      try (FileWriter writer = new FileWriter(outputCountryContextFile, false)) {
        String colNamesForCountryContextFile = "countrycode\tprovcode\tcountycode\tcountryname\t"
            + "provincename\tcountyname\tcountryregex\tprovregex\tcountyregex\n";
