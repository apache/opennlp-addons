 ...
 * self-test samples. Unknown fields are skipped, and malformed input throws
 * {@link InvalidFormatException}.</p>
 *
 * @see <a href=
...
final class ModelProtoReader {
  ...
  /**
   * Prepares a reader positioned at the start of the given bytes.
   *
   ...
   *
   * @param data The raw bytes of a {@code .model} file; must not be {@code null}.
   ...
   */
  private int wireTypeOf(long tag) {
    return (int) (tag & 7);
  }
   ...
   * payload.
   *
   * @param tag The field tag; its wire type must be length-delimited.
   ...
   *
   * @param tag The field tag; its wire type must be varint.
   ...
   *
   * @param tag The field tag; its wire type must be 32-bit.
  ...
  /**
   * Creates the exception for malformed input, including the current byte position.
   *
