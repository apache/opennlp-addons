 ...
 * produces an unused piece is re-segmented into its constituents.</p>
 */
final class BpeEncoder implements Serializable {
   ...
   * @param userDefinedMatcher Longest-match trie over user-defined symbols, or {@code null} when the
   *                           model defines none.
   */
   ...
   *
   * @param normalized The buffer holding the normalized UTF-8 bytes; must not be {@code null}.
   * @param size       The number of valid bytes in {@code normalized}.
   * @return The segments covering all bytes, in text order.
   * @throws IllegalArgumentException Thrown if {@code normalized} is {@code null}.
   */
  List<Segment> encode(byte[] normalized, int size) {
    ...
    // The symbol list uses linked ranges of normalized bytes; merged symbols become empty ranges.
    // User-defined symbols are marked so they are not merged.
    final IntBuilder fromB = new IntBuilder(size);
    final IntBuilder toB = new IntBuilder(size);
    final ByteBuilder mergeBlockedBuilder = new ByteBuilder(size);
    int position = 0;
    while (position < size) {
      int matched = 0;
      ...
      final boolean mergeBlocked = matched > 0;
      final int length = mergeBlocked ? matched
          : Math.min(SentencePieceNormalizer.utf8Length(normalized[position]),
              size - position);
      fromB.append(position);
      toB.append(position + length);
      mergeBlockedBuilder.append(mergeBlocked ? (byte) 1 : (byte) 0);
      position += length;
    }
    final int symbolCount = mergeBlockedBuilder.length();
    final int[] from = fromB.toArray();
    final int[] to = toB.toArray();
    final byte[] mergeBlockedFlags = mergeBlockedBuilder.array();
    final int[] prev = new int[symbolCount];
    final int[] next = new int[symbolCount];
    final boolean[] mergeBlocked = new boolean[symbolCount];
    for (int i = 0; i < symbolCount; i++) {
      prev[i] = i - 1;
      next[i] = i + 1 < symbolCount ? i + 1 : -1;
      mergeBlocked[i] = mergeBlockedFlags[i] != 0;
    }
    ...
    for (int left = 0; left + 1 < symbolCount; left++) {
      maybeAddPair(normalized, from, to, mergeBlocked, left, left + 1, agenda, revMerge);
    }
    ...
    while (!agenda.isEmpty()) {
      ...
      maybeAddPair(normalized, from, to, mergeBlocked, prev[top.left()], top.left(), agenda,
          revMerge);
      maybeAddPair(normalized, from, to, mergeBlocked, top.left(), next[top.left()], agenda,
          revMerge);
    }
   ...
   * @param mergeBlocked Per symbol, whether it is a user-defined symbol excluded from merging.
   ...
   */
  private void maybeAddPair(byte[] normalized, int[] from, int[] to, boolean[] mergeBlocked,
                            int left, int right, PriorityQueue<Pair> agenda,
                            Map<String, String[]> revMerge) {
    if (left == -1 || right == -1 || mergeBlocked[left] || mergeBlocked[right]) {
      return;
    }
    final String piece =
