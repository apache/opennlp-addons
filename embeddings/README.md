...
`embeddings` includes `embeddings-core` and the default
`embeddings-onnx` provider. CLI commands are distributed separately as
`embeddings-cli`; they are also included in the binary distribution.

For static inference without ONNX, depend directly on `embeddings-core`, or
...
<dependency>
  ...
  <exclusions>
    <exclusion>
      <groupId>org.apache.opennlp.addons</groupId>
      <artifactId>embeddings-onnx</artifactId>
    </exclusion>
  </exclusions>
</dependency>
```

Existing `StaticEmbeddingModel.load` calls are unchanged. Use the core registry
with `Providers.of(TextEmbedderProvider.class)` and a `ProviderSpec`:

```java
var providers = Providers.of(TextEmbedderProvider.class);
var spec = ProviderSpec.of(Path.of("/path/to/model-directory"), Map.of());
try (TextEmbedder model = providers.select(spec).create(spec)) {
  float[] vector = model.embed("coffee");
}
```

Discovery uses the spec alone, without opening model files. The `static` provider
accepts local paths without an `.onnx` suffix and no options; `create` validates
the directory and its files. The `onnx` provider accepts local `*.onnx` paths and
its vocabulary, casing, pooling, normalization and maximum-length options. The
highest-priority matching provider wins; the bundle installs both providers.
`providers.byName("static")` returns an optional factory by name. The property
`opennlp.provider.opennlp.tools.embeddings.TextEmbedderProvider` pins selection
to a provider that supports the spec. Each created model belongs to its caller.
The direct `StaticEmbeddingModel.load` method also accepts directories whose
names end with `.onnx`, which registry routing reserves for ONNX models.
...
```sh
mvn -pl embeddings-core -am -Pjmh \
...
The executable [ModelDistillerExampleTest](../embeddings-onnx/src/test/java/opennlp/embeddings/ModelDistillerExampleTest.java)
...
`java embeddings-core/src/test/java/opennlp/embeddings/TestTeacherGenerator.java` from the repository root.
...
- The Dev Manual chapter (`opennlp-addons-docs/src/docbkx/embeddings.xml`) for the same material in the manual.
