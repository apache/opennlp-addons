...
class CorefLicenseIT {
  ...
  void packagedDataRetainsLicenseAndAttribution() throws IOException {
    ...
    try (JarFile jar = new JarFile(path)) {
      assertNotNull(jar.getJarEntry("opennlp/tools/coref/first-names-en.txt"));
      assertTrue(read(jar, "META-INF/LICENSE").contains("Apache License"));
      assertTrue(read(jar, "META-INF/LICENSE").contains("Social Security Administration"),
          "The generated license must retain the bundled data provenance");
      assertTrue(read(jar, "META-INF/NOTICE").contains("Social Security Administration"));
    }
  }
