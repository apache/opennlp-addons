...
final class SentencePieceNormalizer implements Serializable {
   ...
   *                                through without normalization, or {@code null} when the model
   *                                defines none.
   * @throws InvalidFormatException Thrown if the character map has an invalid structure.
   */
   ...
   * contains {@code length + 1} entries.
   */
   ...
   *
   * @param input       The buffer holding well-formed UTF-8 bytes; must not be {@code null}.
   ...
   */
  private void appendSpace(ByteBuilder normalized, IntBuilder normToOrig,
   ...
   */
  private boolean isMalformed(byte[] input, int from, int length) {
   ...
   */
  private int codePointAt(byte[] input, int from, int length) {
   ...
   */
  private int minimalUtf8Length(int codePoint) {
    if (codePoint < 0x80) {
      return 1;
    }
    if (codePoint < 0x800) {
