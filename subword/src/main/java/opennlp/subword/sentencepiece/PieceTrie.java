...
final class PieceTrie implements Serializable {
   ...
   *
   * @param pieces The UTF-8 bytes of each piece; must not be {@code null} or contain empty keys.
  ...
  static PieceTrie build(byte[][] pieces, int[] ids) {
    ...
    // The count and fill passes traverse the same sorted keys with the same recursion.
   ...
   *
   * @param input       The UTF-8 input buffer; must not be {@code null}.
