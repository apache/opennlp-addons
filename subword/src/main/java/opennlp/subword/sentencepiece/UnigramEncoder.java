...
final class UnigramEncoder implements Serializable {
  ...
  List<Segment> encode(byte[] normalized, int size) {
    ...
    // The best path ending at each byte position (exclusive end), stored as
    // [startsAt, raw score bits, id] triples.
    ...
    while (startsAt < size) {
      ...
      if (bestScoreTillHere < -SCORE_RESET_THRESHOLD
        ...
        // Subtracts the same base score from each reachable position to retain float precision on
        // long inputs without changing the best path.
